...
#title of the input article, around which the hyperlink network is built
name = sys.argv[1]

#if True, the input article is included in the network, otherwise it is omitted
include_ego = False
...
page_list_file = codecs.open(data_folder + name + '_articles.txt', 'w', 'UTF-8')
...
def main():	
	...
	for target in outlinks_checked:
		...
		if include_ego and target != title:
			outlinks_file.write( title + '\t' + target  + '\n')  #'\t'.join(map(str, (p, title, pl['title']))) + '\n')				

	if verbose: print '%d articles found, linked from + "%s"' %(local_outdegree, name)
	...
	page_list = outlinks_checked
	if include_ego: page_list += name
	...
	for title in page_list:
		...
		for target in outlinks_checked:
			outlinks_all_file.write( title + '\t' + target + '\n')
			if target in page_list and target != title:
				local_outdegree += 1
				outlinks_file.write( title + '\t' + target  + '\n')  #'\t'.join(map(str, (p, title, pl['title']))) + '\n')				
